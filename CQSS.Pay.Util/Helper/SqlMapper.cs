...
#pragma warning restore 612, 618
    ...
    sealed class DataTableHandler : System.SqlMapper.ITypeHandler
    {
    ...
    /// Used to pass a DataTable as a TableValuedParameter
    /// </summary>
    sealed partial class TableValuedParameter : System.SqlMapper.ICustomQueryParameter
    {
    ...
    /// </summary>
    sealed partial class DbString : System.SqlMapper.ICustomQueryParameter
    {
        /// <summary>
    ...
    sealed partial class DefaultTypeMap : SqlMapper.ITypeMap
    {
        ...
        public ConstructorInfo FindConstructor(string[] names, Type[] types)
        {
            ...
            //优先级 1. Public > internal > Private; 2. 参数个数（少的排前面）
            foreach (ConstructorInfo ctor in constructors.OrderBy(c => c.IsPublic ? 0 : (c.IsPrivate ? 2 : 1)).ThenBy(c => c.GetParameters().Length))
            {
                ...
                for (; i < ctorParameters.Length; i++)
                {
                    ...
                    if (!String.Equals(ctorParameters[i].Name, names[i], StringComparison.OrdinalIgnoreCase))
                        break;
                    // 输入参数类型等于 byte[] 且 构造函数参数类型全名等于 System.Data.Linq.Binary
                    if (types[i] == typeof(byte[]) && ctorParameters[i].ParameterType.FullName == SqlMapper.LinqBinary)
                        continue;
                    // 尝试获取构造函数参数的基础数据类型
                    var unboxedType = Nullable.GetUnderlyingType(ctorParameters[i].ParameterType) ?? ctorParameters[i].ParameterType;

                    if (unboxedType != types[i] // 构参数据类型 不等于 入参数据类型；
                        && !(unboxedType.IsEnum && Enum.GetUnderlyingType(unboxedType) == types[i]) // 构参数据类型为枚举 且 枚举基础类型 等于 入参数据类型
                        && !(unboxedType == typeof(char) && types[i] == typeof(string))) // 构参数据类型等于typeof(char) 且 入参数据类型等于typeof(string)
                        break;
                }
